...
func TestMsg(t *testing.T) {
	...
	PrJSON(`{"name":  {"first":"Tom","last":"Anderson"}, "heve id":true,  "age":37, "children": 
	...
	Note("JSON  -->> ", JSON([]byte(`{"first":"Tom","last":"Anderson"}`)))
	Note("color -->> ", JSONcolor([]byte(`{ "first":"Tom",  "last":"Anderson" }`)))
	Note("line  -->> ", JSONline([]byte(`{ "first":"Tom", "last":"Anderson" }`)))
